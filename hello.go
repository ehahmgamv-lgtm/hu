...
func StartHelloWorldServer(log *zerolog.Logger, listener net.Listener, shutdownC <-chan struct{}) error {
	...
	muxer.HandleFunc(MetricsRoute, metricsHandler())
...
func metricsHandler() http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		mu.RLock()
		isCached := time.Since(last) < time.Second
		data := cache
		mu.RUnlock()

		if isCached && len(data) > 0 {
			w.Write(data)
			return
		}
		...
		mu.Lock()
		defer mu.Unlock()
		if time.Since(last) < time.Second && len(cache) > 0 {
			w.Write(cache)
			return
		}

		resp, err := client.Get("http://127.0.0.1:20241/metrics")
		if err != nil {
			if len(cache) > 0 {
				w.Write(cache)
			} else {
